...
class _QueryBuilder:
    """Proxy for a dialect instance that wires worker/dialect onto returned Queries.

    All methods mirror ``SqlDialectAbstract`` and return fully-wired ``Query``
    objects so callers do not need to configure them manually.
    ...
    def _wrap(self, result: str) -> "Query":
        return Query(query=result, dialect=self._dialect).set_worker(self._worker)
    ...
    def agg(  # pylint: disable=too-many-arguments
        ...
        Args:
            ...
            percentile_ignore_values_: Mapping of column name to values to exclude.
    ...
    def get_duplicates(self, keys: List[str]) -> "Query":
        ...
        Returns:
            A Query containing only the duplicated rows.
        """
        return self._wrap(self._instance.get_duplicates(keys))
    ...
    def jsonify(self, keys: List[str], columns: List[str]) -> "Query":
        ...
        """
        return self._wrap(self._instance.jsonify(keys, columns))
    ...
    def sample(self, num_samples: int) -> "Query":
        ...
        """
        return self._wrap(self._instance.sample(num_samples))
    ...
    def last_partitions(
        ...
        """
        return self._wrap(self._instance.last_partitions(date_ingestion, columns))
    ...
    def enrich(
        ...
        Args:
            ...
            keys: Join condition. Each element is either a column name or a
                ``(left_col, right_col)`` tuple.
            prefix: Optional prefix for columns from the joined side(s).

        Returns:
            A Query wrapping the enriched result set.
        """
        return self._wrap(self._instance.enrich(other, keys, prefix))
    ...
    def get_diffs(
        ...
        Args:
            ...
            keys: Join keys. Each element is a column name or a
                ``(left_col, right_col)`` tuple.
            columns: Columns to compare. Each element is a column name or a
                ``(left_col, right_col)`` tuple.

        Returns:
            A Query showing the differing rows and their values.
        """
        return self._wrap(self._instance.get_diffs(other, keys, columns))
    ...
    def op(
        ...
        Args:
            ...
            select_only: Keep only these columns.
...
class Query:
    ...
    def from_table_name(
        ...
        """
        return cls(
            query=dialect(input_query=table_name).select(table_name),
            dialect=dialect,
        )
